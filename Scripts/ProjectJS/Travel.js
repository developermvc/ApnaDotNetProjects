...
function addFarerow() {
    ...
    $("#fareexp tbody").append(
        ...
        '<td align="center"><input type="text" id="depdate_' + q + '" name="depdate_' + q + '" class="form-control DatePicker" readonly="readonly"/></td>' +
        '<td align="center"><select id="tostation_' + q + '" name="tostation_' + q + '" class="form-control">' + $("#HiddenCityId").html() + '</select ></td>' +
        '<td align="center"><input type="text" id="arrivaldate_' + q + '" data-fromdate="depdate_' + q + '" name="arrivaldate_' + q + '" class="form-control DatePicker" readonly="readonly"/></td>' +
...
function savetraveldetails(input) {
    ...
    $('#lodegingexp tbody tr').each(function (i, tr) {
        ...
        lodegingarr[i] = {
            lodgingfromdate: ($(tr).find("td:nth-child(2) input[type=text]")).val(),
            lodgingtodate: ($(tr).find("td:nth-child(3) input[type=text]")).val(),
            nightspent: ($(tr).find("td:nth-child(4) input[type=text]")).val(),
            lodgingamt: ($(tr).find("td:nth-child(5) input[type=text]")).val(),
            daAmt: ($(tr).find("td:nth-child(6) input[type=text]")).val(),
            lodgingremark: ($(tr).find("td:nth-child(7) input[type=text]")).val(),
            totalamt: ($(tr).find("td:nth-child(8) input[type=text]")).val(),
        }
...
function getlodgingtotal() {
    ...
    $('#lodegingexp tbody tr').each(function (i, tr) {
        if ($(tr).find("td:nth-child(8) input[type=text]").val() != '') {
            ldngtotal = parseFloat(ldngtotal) + parseFloat($(tr).find("td:nth-child(8) input[type=text]").val());
        }
    });
    return ldngtotal;
}
...
function TiffinDateValidation(input) {
    var sdt =   new Date($("#travelExpHDRMaster_fromDate").val());   
    var enddt = new Date($("#travelExpHDRMaster_toDate").val());   
    var cdate = new Date($(input).val());
   r alert($("#travelExpHDRMaster_fromDate").val());
    alert($("#travelExpHDRMaster_toDate").val());
    alert(cdate);
